...
const chatbotResponses = {
  ...
  account: {
    response: "Pour gérer votre compte, veuillez vous connecter via l'interface utilisateur. Vous pouvez modifier vos informations personnelles, voir vos rendez-vous et accéder à vos documents.",
    followUpQuestions: ["Comment modifier mon compte ?", "Comment voir mes rendez-vous ?", "Où sont mes documents médicaux ?"]
  },
  technical_support: {
    response: "Si vous rencontrez des problèmes techniques, veuillez contacter notre assistance à support@i-sgrm.com ou au (+212) 666-666-666.",
    followUpQuestions: ["J'ai un bug sur le site", "Je n'arrive pas à me connecter", "L'application ne fonctionne pas"]
  }
};
...
const intentKeywords = {
  ...
  account: ["compte", "profil", "connexion", "identifiant", "mon compte", "inscription", "mot de passe", "se connecter"],
technical_support: ["bug", "erreur", "problème", "technique", "support", "aide", "assistance", "connexion impossible"]
...
const getSpecificDoctorAvailability = async (message) => {
  try {
    ...
    for (const doctor of doctors) {
      ...
      //   break;
      // }
      const normalizedName = doctorName.replace(/dr\.?|docteur/gi, '').trim();
      if (new RegExp(`\\b${normalizedName}\\b`, 'i').test(lowerMessage)) {
        targetDoctor = doctor;
        break;
      }
    }
